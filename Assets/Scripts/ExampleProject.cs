...
public class ExampleProject : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        degrees = degreesPlane;
        radians = degrees * Mathf.Deg2Rad;
        planeNormal = new Vector3(Mathf.Cos(radians), 0.0f, Mathf.Sin(radians));
    ...
    void OnDrawGizmosSelected()
    {
        ...
        Gizmos.DrawLine(transform.position - new Vector3(0, 0, 1.75f), transform.position + new Vector3(0, 0, 1.75f));

        // Display the plane.
        Gizmos.color = Color.green;
        Vector3 angle = new Vector3(-1.75f * Mathf.Sin(radians), 0, 1.75f * Mathf.Cos(radians));
